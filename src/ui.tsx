...
import React, { ChangeEvent, useContext, useEffect, useLayoutEffect, useMemo, useRef, useState, useReducer } from 'react'
...
class WebSerialDriver implements Driver {
  ...
  public static async connect (port?: SerialPort, hardware: Hardware = 'v3'): Promise<WebSerialDriver> {
    if (port == null) { port = await navigator.serial.requestPort({ filters: [{ usbVendorId: 0x04D8, usbProductId: 0xFD92 }] }) }
  ...
  public async plot (plan: Plan): Promise<void> {
    ...
    if (this._cancelRequested) {
      ...
      if (!penIsUp) {
        ...
        const penUpPosition = (penMotion != null) ? Math.max(penMotion.initialPos, penMotion.finalPos) : device.penPctToPos(50)
  ...
  public limp (): void {
    this.ebb.disableMotors().catch(reason => {
      console.error(`Error disabling motor: ${reason}`)
    })
  }
}
...
class SaxiDriver implements Driver {
  ...
  public limp (): void { this.send({ c: 'limp' }) }
  public ping (): void { this.send({ c: 'ping' }) }
}

const usePlan = (paths: Vec2[][] | null, planOptions: PlanOptions): { isPlanning: boolean, plan: Plan | null, setPlan: React.Dispatch<React.SetStateAction<Plan | null>> } => {
  ...
  function attemptRejigger (previousOptions: PlanOptions, newOptions: PlanOptions, previousPlan: Plan): Plan | undefined {
  ...
  const lastPaths = useRef<Vec2[][] | null>(null)
  const lastPlan = useRef<Plan | null>(null)
  const lastPlanOptions = useRef<PlanOptions | null>(null)
...
const setPaths = (paths: Svg): Record<string, any> => {
  ...
  const groupLayers = Array.from(groups).sort((a, b) => b - a)
  const strokeLayers = Array.from(strokes).sort((a, b) => b - a)
...
function PenHeight ({ state, driver }: { state: State, driver: Driver }): React.ReactElement {
  const { penUpHeight, penDownHeight, hardware } = state.planOptions
  const dispatch = useContext(DispatchContext)
  const setPenUpHeight = (x: number): void => dispatch({ type: 'SET_PLAN_OPTION', value: { penUpHeight: x } })
  const setPenDownHeight = (x: number): void => dispatch({ type: 'SET_PLAN_OPTION', value: { penDownHeight: x } })
...
function HardwareOptions ({ state }: { state: State }): React.ReactElement {
  const dispatch = useContext(DispatchContext)
  const setHardware = (hardware: string): void => dispatch({
...
function VisualizationOptions ({ state }: { state: State }): React.ReactElement {
  const dispatch = useContext(DispatchContext)
...
function SwapPaperSizesButton ({ onClick }: { onClick: () => void }): React.ReactElement {
  return (
    <svg
...
function PaperConfig ({ state }: { state: State }): React.ReactElement {
  const dispatch = useContext(DispatchContext)
  const landscape = state.planOptions.paperSize.isLandscape
  function setPaperSize (e: ChangeEvent): void {
  ...
  function setCustomPaperSize (x: number, y: number): void {
    dispatch({ type: 'SET_PLAN_OPTION', value: { paperSize: new PaperSize({ x, y }) } })
  }
  const { paperSize } = state.planOptions
  const paperSizeName = Object
    .keys(PaperSize.standard)
    .find((psName) => {
      const ps = PaperSize.standard[psName].size
      return (ps.x === paperSize.size.x && ps.y === paperSize.size.y) || (ps.y === paperSize.size.x && ps.x === paperSize.size.y)
    }) ?? 'Custom'
  return (
    <div>
      <select
      ...
      <div>
        <label>
          ...
          <div className='horizontal-labels'>
            ...
            <input
              type='number' min='-90' step='90' max='360' placeholder='0' value={state.planOptions.rotateDrawing}
              onInput={(e) => {
...
function MotorControl ({ driver }: { driver: Driver }): React.ReactElement {
  return (
    <div>
...
function PlanStatistics ({ plan }: { plan: Plan }): React.ReactElement {
  return (
...
function TimeLeft ({ plan, progress, currentMotionStartedTime, paused }: {
  ...
  if (paused || plan?.duration === null || progress === null || currentMotionStartedTime === null) return null
...
function LayerSelector ({ state }: { state: State }): React.ReactElement | null {
  const dispatch = useContext(DispatchContext)
...
function PlotButtons (
  ...
  function plot (plan: Plan): void {
    driver.plot(plan)
  }

  const Planning = (): React.ReactElement => <button className='replan-button' disabled> Replanning... </button>
  const Plan = (): React.ReactElement => (
    <button
      ...
      onClick={() => { if (plan != null) plot(plan) }}
    >
      {(plan != null) && state.progress != null ? 'Plotting...' : 'Plot'}
    </button>
  )
...
function ResetToDefaultsButton (): React.ReactElement {
  const dispatch = useContext(DispatchContext)
  const onClick = (): void => {
...
function PlanOptions ({ state }: { state: State }): React.ReactElement {
  const dispatch = useContext(DispatchContext)
  return (
    <div>
      <form>
...
function PortSelector ({ driver, setDriver, hardware }: PortSelectorProps): React.ReactElement {
  ...
  return (
    <>
      {(driver != null) ? `Connected: ${driver.name()}` : null}
      {(driver == null)
        ? (
          <button
            disabled={initializing}
            onClick={() => {
              navigator.serial.requestPort({ filters: [{ usbVendorId: 0x04D8, usbProductId: 0xFD92 }] })
                .then(async port => {
                  const driver = await WebSerialDriver.connect(port, hardware)
                  setDriver(driver)
                })
          ...
          >
            ...
            {initializing ? 'Connecting...' : (driver != null ? 'Change Port' : 'Connect')}
          </button>
          )
        : null}
    </>
...
function Root (): React.ReactElement {
  ...
  useEffect(() => {
    ...
    if (IS_WEB) {
      setDriver(null as unknown as Driver)
    } else {
  ...
  useEffect(() => {
    const ondrop = (e: DragEvent): void => {
      e.preventDefault()
      if (e == null || e.dataTransfer == null || e.dataTransfer.items.length === 0) return
    ...
    const onpaste = (e: ClipboardEvent): void => {
      if (e == null || e.clipboardData == null || e.clipboardData.items.length === 0) return
  ...
  const showDragTarget = plan == null && !isLoadingFile && !isPlanning
...
function DragTarget (): React.ReactElement {
  return (
