...
export class EBB {
  ...
  private readonly error: Vec2 = { x: 0, y: 0 }
  ...
  public async lowlevelMove (
    ...
    return await this.command(`LM,${initialRate1},${stepsAxis1},${deltaR1},${initialRate2},${stepsAxis2},${deltaR2}`)
  }
   ...
   */
  public async moveWithAcceleration (xSteps: number, ySteps: number, initialRate: number, finalRate: number): Promise<void> {
    ...
    return await this.lowlevelMove(
   ...
   */
  public async moveAtConstantRate (duration: number, x: number, y: number): Promise<void> {
    return await this.command(`XM,${Math.floor(duration * 1000)},${x},${y}`)
  }
  ...
  public async executePenMotion (pm: PenMotion): Promise<void> {
    ...
    return await this.setPenHeight(pm.finalPos, 0, Math.round(pm.duration() * 1000 + 0))
  }

  public async executeMotion (m: Motion): Promise<void> {
    if (m instanceof XYMotion) {
      return await this.executeXYMotion(m)
    } else if (m instanceof PenMotion) {
      return await this.executePenMotion(m)
    } else {
